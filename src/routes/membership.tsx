  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-5 px-4 py-6 sm:space-y-6 sm:px-6 sm:py-8">
      ...
      {currentTier !== "basic" && exp && (
        <Card className="border-emerald-500/40 bg-emerald-500/10 p-4 text-center">
          ...
          <p className="mt-1 font-bold text-emerald-700">
            باقتك الحالية: {TIER_LABELS[currentTier] ?? plans.find((p) => p.tier === currentTier)?.name_ar ?? currentTier}، سارية حتى {exp}
          </p>
        </Card>
      )}

      {!status && <div className="text-center text-muted-foreground">جار التحميل...</div>}
            ...
            <Card key={p.tier} className={`relative min-w-0 space-y-3 border-2 ${meta.ring} p-4 sm:p-6`}>
              ...
              {isFree ? (
                ...
                <Button
                  ...
                  className="w-full"
                >
                  {isCurrent ? "مفعلة" : loading === p.tier ? "جار المعالجة..." : (currentTier === "basic" ? "اشترك" : "ترقية")}
                </Button>
              )}
            </Card>
          );
      ...
      <Card className="p-4 text-sm text-muted-foreground">
        ...
        <p className="mt-1">يتم خصم رسوم الباقة تلقائيا من محفظتك يوم انتهاء العضوية. عند نقص الرصيد تعود عضويتك للوضع المجاني وسنرسل تنبيها بالبريد.</p>
      </Card>
    </div>
  );
}

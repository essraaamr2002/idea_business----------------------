...
export const Route = createFileRoute("/membership")({
  ...
  errorComponent: ({ error }) => <div className="p-8 text-center text-destructive">{(error as Error).message}</div>,
...
function MembershipPage() {
  ...
  const refresh = () => Promise.all([
    ...
    listPlans().then((p: any) => setPlans(p ?? [])).catch(() => setPlans([])),
  ]);
  useEffect(() => { refresh(); }, []);
  ...
  const handleSubscribe = async (tier: "silver" | "gold" | "platinum") => {
    setLoading(tier);
    try {
      const res: any = await subscribe({ data: { tier } });
      if (res?.ok) {
        toast.success(`تم تفعيل العضوية ✅ سارية حتى ${new Date(res.expires_at).toLocaleDateString("ar")}`);
        refresh();
      } else if (res?.reason === "insufficient") {
        const intent: any = await createIntent({ data: { tier } });
        if (intent?.checkoutUrl) {
          window.location.href = intent.checkoutUrl;
        } else {
          toast.error(`الرصيد غير كافٍ (${res.balance} من ${res.needed} ر.س). شحن المحفظة مطلوب.`);
        }
      } else {
        toast.error("تعذّر إتمام الاشتراك");
      }
    } catch (e: any) {
      ...
      {currentTier !== "basic" && exp && (
        <Card className="border-emerald-500/40 bg-emerald-500/10 p-4 text-center">
          ...
          <p className="mt-1 font-bold text-emerald-700">
            باقتك الحالية: {plans.find(p => p.tier === currentTier)?.name_ar ?? currentTier} — سارية حتى {exp}
          </p>
        </Card>
      )}

      {!status && <div className="text-center text-muted-foreground">جارٍ التحميل…</div>}
        ...
        {plans.map((p) => {
          ...
          return (
            <Card key={p.tier} className={`relative space-y-3 border-2 ${meta.ring} p-6`}>
              {p.tier === "gold" && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-primary px-3 py-0.5 text-[10px] font-bold text-primary-foreground">الأكثر اختياراً</span>
              )}
              <div className="flex items-center justify-between">
                <h2 className="flex items-center gap-2 text-lg font-bold">
                  <Icon className="h-5 w-5" /> {p.name_ar}
                </h2>
                {isCurrent && <Badge variant="secondary">حالية</Badge>}
              </div>

              <p className="text-3xl font-extrabold tabular-nums">
                {isFree ? "مجاني" : <>{Number(p.price_sar).toLocaleString("ar")} <span className="text-xs font-normal text-muted-foreground">ر.س / شهر</span></>}
              </p>

              <ul className="space-y-1.5 text-xs">
                <Feature label={`مشاريع: ${fmtCap(p.projects_cap)}`} unl={p.projects_cap < 0} />
                ...
                {p.verified_badge && <Feature label="صح أزرق ✓ بجوار اسمك" />}
                {p.priority_support && <Feature label="أولوية في الدعم" />}
                {p.ai_advanced && <Feature label="مساعد AI متقدّم" />}
                {p.dedicated_manager && <Feature label="مدير حساب مخصّص" />}
              </ul>
              ...
              {isCurrent && (
                <div className="rounded-lg bg-muted/50 p-2 text-[11px]">
                  ...
                  <div className="mt-1 grid grid-cols-2 gap-1 tabular-nums">
                    <span>مشاريع {usage.projects}/{fmtCap(p.projects_cap)}</span>
                    <span>إعجابات {usage.likes}/{fmtCap(p.likes_cap)}</span>
                    <span>تعليقات {usage.comments}/{fmtCap(p.comments_cap)}</span>
                    <span>أخرى {usage.other}/{fmtCap(p.other_cap)}</span>
                  </div>
                </div>
              )}

              {isFree ? (
                <Button variant="outline" disabled className="w-full">الباقة الافتراضية</Button>
              ) : (
                <Button
                  ...
                  className="w-full"
                >
                  {isCurrent ? "مفعّلة" : loading === p.tier ? "جارٍ المعالجة…" : (currentTier === "basic" ? "اشترك" : "ترقية")}
                </Button>
              )}
            </Card>
          );
      ...
      {status && (
        <Card className="p-4 text-xs text-muted-foreground">
          <p className="flex items-center gap-1">
            ...
            <Link to="/wallet" className="text-primary underline">شحن المحفظة</Link>
          </p>
        </Card>
      )}

      <Card className="p-4 text-sm text-muted-foreground">
        <p className="font-bold text-foreground flex items-center gap-1"><Zap className="h-4 w-4" /> كيف يعمل التجديد التلقائي؟</p>
        <p className="mt-1">يتم خصم رسوم الباقة تلقائياً من محفظتك يوم انتهاء العضوية. عند نقص الرصيد تعود عضويتك للوضع المجاني وسنرسل تنبيهاً بالبريد.</p>
      </Card>
    </div>
  );
}

...
DROP TRIGGER IF EXISTS trg_protect_profiles_ins ON public.profiles;
DROP TRIGGER IF EXISTS trg_protect_profiles_upd ON public.profiles;
CREATE TRIGGER trg_protect_profiles_ins BEFORE INSERT ON public.profiles FOR EACH ROW EXECUTE FUNCTION public.protect_profiles_privileged_cols();
CREATE TRIGGER trg_protect_profiles_upd BEFORE UPDATE ON public.profiles FOR EACH ROW EXECUTE FUNCTION public.protect_profiles_privileged_cols();
